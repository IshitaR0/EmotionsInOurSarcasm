...
# Ensure we only use the first 650 labels
y_train[:650] = labels_array[:650]  # Your 650 labeled instances
...
labeled_accuracy = accuracy_score(labels_array[:650], predicted_labels[:650])
...
final_labels[:650] = labels_array[:650]  # Keep original labels
final_labels[650:] = np.where(
    high_confidence_mask[650:],
    predicted_labels[650:],
    -1  # Keep as unlabeled if low confidence
)

# Count how many new labels we assigned
newly_labeled = np.sum(final_labels[650:] != -1)
print(f"Newly labeled instances: {newly_labeled} out of {len(final_labels) - 650}")
...
def get_samples_for_annotation(label_distributions, n_samples=10):
    ...
    # Only consider currently unlabeled data (index 650 onwards)
    unlabeled_entropies = entropies[650:]
    
    # Get indices of most uncertain samples
    most_uncertain = np.argsort(unlabeled_entropies)[-n_samples:]
    
    # Return original indices (adding 650)
    return most_uncertain + 650
...
for i in range(4):  # Four emotion classes
    mask = labels_array[:650] == i
    plt.scatter(
        embeddings_2d[:650][mask][:, 0],
        embeddings_2d[:650][mask][:, 1],
        color=colors[i],
...
# Plot unlabeled data
unlabeled_mask = final_labels[650:] == -1
plt.scatter(
    embeddings_2d[650:][unlabeled_mask][:, 0],
    embeddings_2d[650:][unlabeled_mask][:, 1],
    color='gray',
    marker='.',
    label='Unlabeled',
    alpha=0.2
