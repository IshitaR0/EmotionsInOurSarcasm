...
# # # ─── HYPERPARAMETERS ──────────────────────────────────────────────────────────
# # TEXT_UTTER_DIM   = 100    # your FastText dim per utterance (100 or 300)
# # TEXT_HID_DIM     = 128
# # VIDEO_HID_DIM    = 128
# # FUSION_OUT_DIM   = 256
# # CLASS_HID_DIM    = 256
...
# #     """
# #     Expects:
# #       - text_csv: each row is one sample, with (seq_len * TEXT_UTTER_DIM) columns
# #       - vid_csv : each row is one sample, with V_DIM columns
# #       - label_csv: one column named 'emotion' with ints 0..C-1
# #     The rows must be aligned in the same order across all three files.
# #     """
...
# #         assert len(df_txt) == len(df_vid) == len(df_lbl), "Mismatched row counts!"
...
# #         # reshape text into (seq_len, TEXT_UTTER_DIM)
# #         t = self.X_txt[i].view(self.seq_len, TEXT_UTTER_DIM)
# #         # reshape video into (1, V_DIM)
# #         v = self.X_vid[i].unsqueeze(0)
# #         return t, v, self.y[i]
...
# #         rep = (h * weights.unsqueeze(-1)).sum(1)       # (B, H)
# #         return rep
...
# #         α   = torch.sigmoid(self.inter(cat))           # (B, 1)
# #         fused = α * t_p + (1 - α) * v_p                # (B, out_dim)
# #         return fused
...
# #         # t_seq: (B, seq_len, TEXT_UTTER_DIM)
# #         # v_seq: (B, 1, video_dim)
# #         t_feat = self.text_enc(t_seq)      # (B, 2*H)
...
# #         tr_loss = train_one_epoch(model, tr_ld, opt, crit, device)
...
# #             print(" → saved best_full_model.pt")
...
# # full_pipeline_flat_with_reg.py
...
#     def __getitem__(self, i):
#         # text as sequence of utterance embeddings
#         t = self.X_txt[i].view(self.seq_len, TEXT_UTTER_DIM)  # (seq_len, D)
#         # video as one-step sequence
...
#         scores  = self.attn(out).squeeze(-1)    # (B, seq_len)
#         weights = torch.softmax(scores, dim=-1) # (B, seq_len)
#         return (out * weights.unsqueeze(-1)).sum(1)  # (B, 2*H)
